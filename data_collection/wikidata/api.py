...
class WikiDataScholars:
  ...
  def get_scholar_advisors_and_students(self, scholar_query_id):
    ...
    try:
      cur_scholar_image = WIKIDATA_IMAGE + cur_scholar.attributes['claims'][property_map['image']][0]['mainsnak']['datavalue']['value'].replace(" ", "_")
    except (TypeError, KeyError):
    ...
    try:
      cur_scholar_field = cur_scholar.attributes['claims'][property_map['field']][0]['mainsnak']['datavalue']['value']['id']
    except (TypeError, KeyError):
